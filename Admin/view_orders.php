...
<?php
	...
	if(isset($_GET['view_id']) && !empty($_GET['view_id']))
	{
		...
		if(isset($_GET['action']) && $_GET['action'] =='finished'){
			$stmt_update = $DB_con->prepare("UPDATE orderdetails set order_status='Ordered_Finished' WHERE order_status = 'Sipariş edildi and  user_id=:user_id");
    ...
    <div id="wrapper">
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="index.php">Market - Yönetici Paneli</a>
            </div>
        ...
        <div id="page-wrapper">
			 ...
			 <div class="alert alert-danger">
                          ...
                          <center> <h3><strong>Ürün Yönetimi</strong> </h3></center>
...
include("config.php");
	...
	if($stmt->rowCount() > 0)
	{
		...
		include("config.php");
		 $stmt_edit = $DB_con->prepare("select sum(order_total) as totalx from orderdetails where user_id=:user_id and order_status='sipariş edildi'");
		...
		echo "<tr>";
		echo "<td colspan='3' align='right' style='font-size:20px;'>Customer: ".$user_firstname." ".$user_lastname." | <span style='color:red'>Sipariş Edilen Toplam Fiyat:</span>";
		echo "</td>";
		...
		echo "</td>";
		 ...
		 echo "<td>";
		 echo "<a class='btn btn-danger' href='customers.php'><span class='glyphicon glyphicon-backward'></span> Geri<a/>'";
		 echo "<a class='btn btn-primary' href='?view_id=".$view_id."&action=finished'><span class='glyphicon glyphicon-check'></span> Bitmiş<a/>'";
		 echo "</td>";
